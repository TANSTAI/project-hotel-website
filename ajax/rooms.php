<?php
    ...
    if(isset($_GET['fetch_rooms']))
    {
        ...
        //phòng check in out hợp lệ

        if($chk_avail['checkin']!='' && $chk_avail['checkin']!='')
        {
        ...
        //fetch setting table to check website for shutdown or not
        $settings_q = "SELECT * FROM `settings` WHERE `sr_no`=1";
        $settings_r = mysqli_fetch_assoc(mysqli_query($con,$settings_q));


        //query for room card with guest filter
        ...
        {
            //check các phòng hiện có thể book
                if($chk_avail['checkin']!='' && $chk_avail['checkin']!='')
                {
            ...
            //
                ...
                if(!$settings_r['shutdown'])
                {
                    ...
                    $book_btn="<button onclick='checkLoginToBook($login,$room_data[id])' class='btn btn-sm w-100 text-white custom-bg shadow-none mb-2'>Book Ngay</button>";
                }
            // print room card
            $output.="
        ...
        if($count_rooms>0){
            ...
            echo "<h3 class='text-center text-danger'>Không có phòng theo yêu cầu của bạn! vui chọn lại.</h3>";
        }
    }
